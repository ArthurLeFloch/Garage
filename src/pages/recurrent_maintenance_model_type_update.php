...
loadJS('./pages/recurrent_maintenance_model_type_update.js');

cardHeader("Modification d'une maintenance récurrente sur la famille de modèle");

$maintenance_id = parseGet('maintenance_id');

$requete = "SELECT model_type_id, intervention_id, mileage_repeat, days_after_last_maintenance
...
<form class="row g-3">
    <div class="col-md-12">
        ...
        <select class="selectpicker form-control" data-live-search="true" id="model-type">
            ...
            while ($models = pg_fetch_array($res)) {
                ...
                if ($models[0] == $model_type_id) {
                    echo "<option value='" . htmlspecialchars($models[0]) . "' selected>" . htmlspecialchars($models[1]) . "</option>";
                } else {
                    echo "<option value='" . htmlspecialchars($models[0]) . "'>" . htmlspecialchars($models[1]) . "</option>";			
                }
            }
    ...
    <div class="col-12">
        <button type="submit" class="btn btn-primary" id="updateMaintenanceRecModelType">Valider</button>
    </div>
</form>
